...
| **Aligner 系统** | 按内容类型路由到专用审核员：Script-Aligner（剧本）、Storyboard-Aligner（分镜）、Plan-Aligner（大纲/计划），返回 PASS/FAIL。详见 `short-drama-reviewer` 技能 v2.0 架构 |
...
> 💡 **长篇小说改编工作流（45分钟/集格式，2026-05-15 基督山伯爵验证）**：
> 当项目是集数较多（15+集）、单集时长较长（30-60分钟）的长篇改编：
> 1. **小说来源**：如AI已知原著（公版经典），可直接利用知识生成大纲，无需 PDF 读取
> 2. **两阶段执行**：先批量完成全部剧本（可用 execute_code/write_file 批量写入），再回头补分镜+Prompts
> 3. **45分钟格式 vs 70秒格式的差异**：
>    | 维度 | 70秒短剧 | 45分钟剧集 |
>    |------|---------|-----------|
>    | 场景数/集 | 10-12 | 18-25 |
>    | 对白数/集 | 12-15句 | 60-100句 |
>    | 单剧本大小 | ~3KB | ~12-15KB |
>    | 单分镜大小 | ~2KB | ~4-5KB |
>    | 单Prompts大小 | ~3KB | ~10-13KB |
>    | 镜头数/集 | 18-22 | 22-25 |
>    | 对话长度 | ≤15字/句 | 自然长度，不限15字 |
> 4. **批量脚本写入模式**（经验证 EP-03~05 三集同时写入成功）：
>    ```python
>    for ep, content in [("EP-03", script_03), ("EP-04", script_04), ("EP-05", script_05)]:
>        write_file(path=f"{BASE}/script/{ep}.md", content=content)
>    ```
> 5. **连续性管理**：25集规模时，continuity.md 每3-5集批量更新一次（非每集更新），EP-15+ 后触发摘要截断
> 6. **分批交付策略**：建议按"部"交付（如先完成第一部5集三件套→确认→推进第二部）
> 7. **单集输出量**：45分钟格式单集三件套 ≈ 25-35K tokens 输出，远超70秒格式的15-17K
> 8. **经典文学改编无需 IP 分析阶段**：直接跳过阶段0→阶段1（大纲），因为 AI 已有原著知识

## 审核员系统（Aligner）

> 审核规则与评分标准维护在 **`short-drama-reviewer` 技能 v2.0** 中（三专用 Aligner：Script / Storyboard / Prompt）。
> 本技能只负责调用，不重复定义规则。审核时加载 `short-drama-reviewer` 即可。
